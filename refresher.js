...
const updateProducts = (googleId, productId, infoList) => {
  if (infoList.isAvailable) {
    console.log(`available ${infoList.title} ${infoList.price}`);
    User.findOneAndUpdate(
      { googleId: googleId, "jumiaProducts._id": productId },
      {
        $set: {
      ...
      (err, data) => {
        ...
        // console.log(data)
        const product = data.jumiaProducts.filter(
          (pro) => pro._id.toString() === productId
        )[0];
        if (product.isSimpleSelected) {
          const oldSimpleSelected = product.simples.filter(
            (simple) => simple.sku === product.simpleSelected
          )[0];
          const newSimpleSelected = infoList.simples.filter(
            (simple) => simple.sku === product.simpleSelected
          )[0];
    ...
    User.findOneAndUpdate(
      { googleId: googleId, "jumiaProducts._id": productId },
      { $set: { "jumiaProducts.$.isAvailable": infoList.isAvailable } },
      (err, data) => {
        if (err) console.log("err : ", err);
      }
    );
...
const getProductInfo = async (productUrl, minPrice) => {
  try {
    const data = await axios.get(productUrl).then((res) => res.data);
    // .catch(err => console.error("catch error ", err.message))
    const json = JSON.parse(
      data
        .split("</script><script>window.__STORE__=")[1]
    ...
    for (const bSimple of bSimples) {
      simples.push({
        sku: bSimple.sku,
        isBuyable: bSimple.isBuyable,
        name: bSimple.name,
        price: Number(bSimple.prices.rawPrice),
        isCheap: minPrice >= Number(bSimple.prices.rawPrice),
      });
    }
    return {
      isAvailable: true,
...
const dbRefresh = setInterval(() => {
  ...
  User.find((err, data) => {
    ...
    for (const user of data) {
      console.log(`updating for user ${user.name}`);
      for (const product of user.jumiaProducts) {
        getProductInfo(product.url, product.minPrice).then((infoList) =>
          updateProducts(user.googleId, product._id.toString(), infoList)
        );
      }
    }
  });
