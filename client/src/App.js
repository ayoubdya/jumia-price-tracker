...
function App() {
  ...
  const handeUrlChange = e => {
    ...
    ((url.match(/^http(s)?:\/\/www\.jumia\.ma\/.+\.html/)) || !url) ? setUrlError(false) : setUrlError(true)
  }
  ...
  const handeMinPriceChange = e => {
    ...
    (Number(dotMinPrice) > 0 || !minPrice) ? setMinPriceError(false) : setMinPriceError(true)
  }

  useEffect(() => {
    if (patchInfo) {
      // if (googleProfile) {
      patchProductMinPrice(patchInfo.id, patchInfo.minPrice, patchInfo.price, token)
  ...
  useEffect(() => {
    (fullURL && fullMinPrice && !minPriceError && !urlError && !isAlert && !isLoading) ? setAllValid(true) : setAllValid(false)
  }, [fullURL, fullMinPrice, minPriceError, urlError, isAlert, isLoading])

  useEffect(() => {
    if (delId) {
      pullProductWithProductId(delId, token)
        .then(profile => setOutput(profile.jumiaProducts))
    }
        ...
        getProductInfoFromApi(change.url, Number(change.minPrice), token)
          .then(productInfo => {
            if (Number(change.minPrice) >= productInfo.price) {
              ...
              setIsAlert("Your minimum price is higher than the current price!")
            }
            pushProductWithGoogleId(productInfo, token)
              .then(profile => {
  ...
  useEffect(() => {
    if (reloadPage) {
      getUserWithGoogleId(token).then(profile => setOutput(profile.jumiaProducts)).catch(() => {
        setOutput(null)
      })
    }
    ...
    onAuthStateChanged(auth, (aUser) => {
      ...
      if (aUser) {
        auth.currentUser.getIdToken()
          .then(tokenId => {
            setToken(tokenId)
            getUserWithGoogleId(tokenId).then(profile => setOutput(profile.jumiaProducts)).catch(err => console.log(err))
          })
      } else {
        setOutput(null)
        setToken(null)
      ...
      <ThemeProvider theme={theme}>
        <AppBar>
          <Toolbar disableGutters={true} className={classes.toolBar} >
            ...
            <Typography variant="button" noWrap={true}> Jumia Price Tracker </Typography>
            <Button sx={{ ml: 'auto', mr: '0.5rem' }} color='secondary' disableElevation={true} onClick={token ? handleGoogleSignOut : handleGoogleSignIn} variant="contained" startIcon={<GoogleIcon />}>
        ...
        <Container maxWidth='lg' className={classes.inputFormContainer}>
          <form onSubmit={handleSubmit(onSubmit)} onKeyDown={(e) => checkKeyDown(e)}>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 1, sm: 1, md: 2 }} justifyContent="space-between">
              <TextField fullWidth {...register('url')} error={urlError} label={urlError ? "Error" : "Product URL"} helperText={urlError ? "Not A Valid URL" : null} variant="outlined" placeholder="https://www.jumia.ma/product.html" onChange={handeUrlChange} />
              <TextField {...register('minPrice')} error={minPriceError} label={minPriceError ? "Error" : "Min Price"} helperText={minPriceError ? "Only Positive Numbers Allowed" : null} variant="outlined" placeholder="00.00" InputProps={{ endAdornment: <InputAdornment position="end">Dh</InputAdornment> }} onChange={handeMinPriceChange} />
              <Button type="submit" variant="outlined" disabled={!allValid}><SearchIcon /></Button>
            </Stack>
          </form>
        </Container>
        ...
        <Container sx={{ minHeight: "90vh" }} maxWidth='lg'>
          <Pagination disabled={isLoading} page={page} onChange={handlePaginationChange} count={output ? Math.ceil(output.length / dataLimit) : 1} sx={{ mb: '1rem', alignItems: "center", justifyContent: "center", display: "flex" }} />
          <Grid container spacing={2}>
            {output && [...output].reverse().slice(index[0], index[1]).map(pro => (<Product token={token} setOutputFromProduct={(outputFromProduct) => setOutput(outputFromProduct)} changePatchId={({ id, minPrice, price }) => { setPatchInfo({ id, minPrice, price }) }} changeDelId={id => setDelId(id)} key={pro._id} pro={pro} />))}
          </Grid>
        </Container>
        <Divider sx={{ my: '1rem' }} />
        <Footer />
