...
export default function Product(props) {
  ...
  const [simpleChange, setSimpleChange] = useState(props.pro.simpleSelected || null)

  const onMinPriceSubmit = (data) => {
    const dotMinPrice = Number(data.minPrice.replace(',', '.'))
    if (dotMinPrice > 0 && ((simpleChange && dotMinPrice < props.pro.simples.filter(simple => (simple.sku === simpleChange))[0].price) || (!simpleChange && dotMinPrice < props.pro.price))) { //OMG
      props.changePatchId({ id: props.pro._id, minPrice: dotMinPrice, price: props.pro.price })
      setMinPriceChangeError(false)
      setIsEditMinPrice(false)
      setMinPrice(dotMinPrice)
      if (simpleChange) {
        const simpleSelected = props.pro.simples.filter(simple => (simple.sku === simpleChange))[0]
  ...
  useEffect(() => {
    if (parentFinalOption) {
      ...
      // .then(profile => {
      const simpleSelected = props.pro.simples.filter(simple => (simple.sku === parentFinalOption))[0]
      ...
      if (minPrice >= simpleSelected.price) {
        ...
        props.changePatchId({ id: props.pro._id, minPrice: simpleSelected.price - 1, price: props.pro.price })
      }
  ...
  return (
    <>
      <Grid item xs={12} sm={6} md={4} >
        <Card className={props.pro.isAvailable ? null : classes.grayCard} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          <CardMedia component="img" sx={{ pt: 0 }} image={props.pro.imageUrl} alt={props.pro.title} />
          <CardContent sx={{ flexGrow: 1, alignItems: 'center', p: '0.25rem' }}>
            <Stack alignItems="center" direction="row" justifyContent="space-between">
              ...
              <IconButton value={props.pro._id} onClick={(e) => props.changeDelId(e.currentTarget.value)} aria-label="delete">
                <DeleteIcon />
              </IconButton>
            </Stack>
            <Stack alignItems="center" direction="row" justifyContent="space-between">
              {props.pro.variationSelection ? (
                <Simples isEditMinPrice={isEditMinPrice} setFinalOption={(option) => setParentFinalOption(option)} pro={props.pro} />
              ) : (
              ...
              {isEditMinPrice ? (
                <form onSubmit={handleSubmit(onMinPriceSubmit)}>
                  <TextField {...register('minPrice')} size="small" variant="outlined" placeholder="00.00" helperText={minPriceChangeError ? "Incorrect Number" : null} label={minPriceChangeError ? "Error" : "Min Price"} error={minPriceChangeError}></TextField>
                </form>
              ) : (
                <Typography variant='subtitle2' color={isCheap ? 'green' : 'red'}>
                  Min: {minPrice} Dh
                </Typography>
              )}
              <IconButton disabled={!props.pro.isAvailable} onClick={(e) => {
          ...
          <CardActions>
            <Rating name="read-only" value={props.pro.stars} precision={0.1} readOnly />
          </CardActions>
        </Card>
      </Grid>
    </>
